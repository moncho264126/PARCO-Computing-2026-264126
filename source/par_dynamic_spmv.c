 ...
 *        4. Performs y = A * x using a CSR kernel with OpenMP parallelization and dynamic scheduling.
...
double csr_vector_multiply(int M, double *values, int *col_idx, int *row_ptr, double *x, double *y, int thread_count, int chunk_size){
    ...
    #pragma omp parallel for num_threads(thread_count) schedule(dynamic, chunk_size) // OpenMP divide las iteraciones del bucle (por ejemplo, filas de la matriz) en bloques de 32 y se las asigna a los hilos a medida que van quedando libres.
                                                                                     // El chunk size = número de iteraciones que cada hilo toma de golpe antes de pedir más trabajo.
    for(int i = 0; i < M; i++){
        double sum = 0.0;
