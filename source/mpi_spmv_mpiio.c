...
int main(int argc, char **argv) {
    ...
    // MPI usa int para el count, así que si bytes_to_read_offset > 2GB, fallará.
    // Leemos en bucle usando MPI_File_read_at (independiente)
    ...
    while (remaining > 0) {
        ...
        // Usamos lectura independiente (_at) en lugar de colectiva (_at_all) 
        // para facilitar el bucle si los tamaños difieren mucho entre procesos.
    ...
    // Malloc de 1GB por proceso si N=128M
    ...
    if (rank == 0) {
        ...
        if (fv) {
            ...
            // Lectura puede ser lenta, pero necesaria
            for(int i=0; i<vm; i++) {
                if(fscanf(fv, "%lf", &x_global[i]) != 1) break;
            }
            fclose(fv);
        } else {
             fprintf(stderr, "Error abriendo vector %s\n", argv[2]);
        }
    }
