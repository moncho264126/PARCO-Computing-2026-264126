 ...
 *        4. Performs y = A * x using a CSR kernel with OpenMP parallelization and automatic scheduling.
 ...
 *
 * Execution:
 *        ./par_auto_SpmV matrix.mtx vector.mtx <threads>
 *
 * Output:
...
double csr_vector_multiply(int M, double *values, int *col_idx, int *row_ptr, double *x, double *y, int thread_count){
    double start = omp_get_wtime();
    #pragma omp parallel for num_threads(thread_count) schedule(auto)  // Le dice al runtime que decida automáticamente cómo dividir las iteraciones del bucle entre los hilos (static, dynamic o guided)
    for(int i = 0; i < M; i++){
        double sum = 0.0;
...
int main(int argc, char *argv[]) {
    if(argc < 4){
        printf("Usage: %s matrix.mtx vector.txt #threads\n", argv[0]);
        return 1;
    }
    ...
    // Multiply
    elapsed = csr_vector_multiply(M, values, col_idx, row_ptr, x, y, thread_count);

    printf("\nElapsed time = %e seconds with %d threads\n\n", elapsed, thread_count);
