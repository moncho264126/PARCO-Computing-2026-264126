 ...
 *    executable `par_static_spmv`.
 *
 *    This script:
 *        1. Compiles the source code (`par_static_spmv.c` and `mmio.c`) with OpenMP support.
...
SOURCE_DIR="./src"
MATRIX_DIR="./matrix"
SOURCE="$SOURCE_DIR/par_static_spmv.c"
MMIO="$SOURCE_DIR/mmio.c"
EXE_NAME="par_static_spmv"
...
for idx in "${SELECTED_INDICES[@]}"; do
    ...
    for THREAD in "${THREADS[@]}"; do
        for CHUNK in "${CHUNKSIZES[@]}"; do
            ...
            RESULT_FILE="$RESULTS_DIR/par_static_${THREAD}_${CHUNK}_${PCT}.txt"
