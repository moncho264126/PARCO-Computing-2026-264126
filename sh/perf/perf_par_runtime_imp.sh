 ...
 *    executable `par_runtime_imp_spmv`.
 *
 *    This script:
 *        1. Compiles the source code (`par_runtime_imp_spmv.c` and `mmio.c`) with OpenMP support.
...
for idx in "${SELECTED_INDICES[@]}"; do
    ...
    for THREAD in "${THREADS[@]}"; do
        for CHUNK in "${CHUNKSIZES[@]}"; do
            ...
            RESULT_FILE="$RESULTS_DIR/par_runtime_imp_${THREAD}_${CHUNK}_${PCT}.txt"
