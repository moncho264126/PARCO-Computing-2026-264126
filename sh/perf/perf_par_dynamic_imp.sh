 ...
 *    executable `par_dynamic_imp_spmv`.
 *
 *    This script:
 *        1. Compiles the source code (`par_dynamic_imp_spmv.c` and `mmio.c`) with OpenMP support.
...
SOURCE_DIR="./src"
MATRIX_DIR="./matrix"
SOURCE="$SOURCE_DIR/par_dynamic_imp_spmv.c"
MMIO="$SOURCE_DIR/mmio.c"
EXE_NAME="par_dynamic_imp_spmv"
...
for idx in "${SELECTED_INDICES[@]}"; do
    ...
    for THREAD in "${THREADS[@]}"; do
        for CHUNK in "${CHUNKSIZES[@]}"; do
            ...
            RESULT_FILE="$RESULTS_DIR/par_dynamic_imp_${THREAD}_${CHUNK}_${PCT}.txt"
