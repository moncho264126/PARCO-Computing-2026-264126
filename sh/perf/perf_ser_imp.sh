 ...
 *    multiplication executable `ser_imp_spmv`.
 *
 *    This script:
 *        1. Compiles the source code (`ser_imp_spmv.c` and `mmio.c`) with OpenMP support enabled.
...
SOURCE="$SOURCE_DIR/ser_imp_spmv.c"
MMIO="$SOURCE_DIR/mmio.c"
EXE_NAME="ser_imp_spmv"
...
for index in "${!MATRICES[@]}"; do
    ...
    RESULT_FILE="$RESULTS_DIR/ser_imp_$(basename "$MATRIX_FILE" .mtx).txt"
    echo "Processing matrix $MATRIX with vector $VECTOR"

    echo "Performance results for $MATRIX" > "$RESULT_FILE"
    echo "run | L1-loads | L1-misses | LLC-loads | LLC-misses" >> "$RESULT_FILE"
