 ...
 *    executable `par_dynamic_spmv`.
 *
 *    This script:
 *        1. Compiles the source code (`par_dynamic_spmv.c` and `mmio.c`) with OpenMP support.
...
for idx in "${SELECTED_INDICES[@]}"; do
    ...
    for THREAD in "${THREADS[@]}"; do
        for CHUNK in "${CHUNKSIZES[@]}"; do
            ...
            RESULT_FILE="$RESULTS_DIR/par_dynamic_${THREAD}_${CHUNK}_${PCT}.txt"
