 ...
 *    executable `par_runtime_spmv`.
 *
 *    This script:
 *        1. Compiles the source code (`par_runtime_spmv.c` and `mmio.c`) with OpenMP support.
...
SOURCE_DIR="./source"
MATRIX_DIR="./matrix"
SOURCE="$SOURCE_DIR/par_runtime_spmv.c"
MMIO="$SOURCE_DIR/mmio.c"
EXE_NAME="par_runtime_spmv"
...
echo "Compiling..."
gcc -std=c99 -g -Wall -fopenmp -o "$BUILD_DIR/$EXE_NAME" "$SOURCE" "$MMIO" -lm
if [ $? -ne 0 ]; then
...
for idx in "${SELECTED_INDICES[@]}"; do
    ...
    for THREAD in "${THREADS[@]}"; do
        for CHUNK in "${CHUNKSIZES[@]}"; do
            ...
            OUTFILE="$RESULTS_DIR/par_runtime_${THREAD}_${CHUNK}_${PCT}.txt"
