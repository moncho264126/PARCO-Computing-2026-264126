...
for i in "${!PROCS_LIST[@]}"; do
    ...
    RESULT_FILE="$RESULTS_DIR/weak_mpiio_np_${NP}.txt"

    echo "Running weak mpiio scaling with NP=$NP"
    echo "Matrix: ${MATRICES[$i]}"

    # Encabezado
    echo "Weak mpiio scaling NP=$NP" > "$RESULT_FILE"
    echo "Run | Total_Time | Communication | Computation" >> "$RESULT_FILE"

    for ((run=1; run<=N_RUNS; run++)); do
        OUT=$(mpirun -np "$NP" "$BUILD_DIR/$EXE_NAME" "$MATRIX" "$VECTOR")

        # Parsing idéntico al strong scaling
        ...
        if [ -z "$T_TOTAL" ]; then
            echo "Error en Run $run: No se capturaron tiempos."
        else
