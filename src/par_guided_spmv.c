 ...
 *        4. Performs y = A * x using a CSR kernel with OpenMP parallelization and guided scheduling.
 ...
 *
 * Compilation:
 *        gcc -g -Wall -fopenmp -o par_guided_spmv par_guided_spmv.c mmio.c
 *
 * Execution:
 *        ./par_guided_spmv matrix.mtx vector.mtx <threads> <chunk_size>
 *
 * Output:
...
double csr_vector_multiply(int M, double *values, int *col_idx, int *row_ptr, double *x, double *y, int thread_count, int chunk_size){
    ...
    #pragma omp parallel for num_threads(thread_count) schedule(guided, chunk_size)  // Los chunks se van reduciendo conforme el bucle avanza
                                                                             // El param 32 es el tam minimo de bloque cuando el bucle está terminado
    for(int i = 0; i < M; i++){
        double sum = 0.0;
