/********************************************************************************************
 * Description:
 *    High-performance serial Sparse Matrix-Vector multiplication (SpMV) benchmark
 *    using the Compressed Sparse Row (CSR) format with aligned memory for SIMD/vectorization.
 *
 *    This program:
 ...
 *        3. Allocates 64-byte aligned memory for CSR arrays and vectors.
 *        4. Reads an input vector from a Matrix Market file into aligned memory.
 *        5. Performs y = A * x using an optimized CSR kernel featuring:
 *               - Loop unrolling for instruction-level parallelism (ILP)
 *               - Aligned memory accesses for cache efficiency
 *        6. Measures total kernel execution time with wall-clock precision.
 *
 * Matrix Format:
 *    Input matrix must be in:
 *        Matrix Market (.mtx), coordinate, real, general/symmetric
 *
 *    CSR arrays generated (64-byte aligned):
 ...
 *
 * Compilation:
 *        gcc -O3 -march=native -o ser_imp_spmv ser_imp_spmv.c mmio.c -lm -fopenmp
 *
 * Execution:
 *        ./ser_imp_spmv matrix.mtx vector.mtx
 *
 * Output:
 ...
 *        - timer.h for portable timing functions
 ********************************************************************************************/
